...
func RegisterShortenerRoutes(server *fiber.App, client *mongo.Client) {
  ...
  limiterMiddleware := limiter.New(limiter.Config{
		...
		LimitReached: func(c *fiber.Ctx) error {
			return c.JSON(fiber.Map{
				"status":      429,
				"message":     "rate limited, contact request@jeanvides.com for get more shorts per minute",
				"data":        nil,
				"exited_code": 1,
			})
		},
	...
	server.Post("/api/v1/post/shortener/create", limiterMiddleware, func(c *fiber.Ctx) error {
		...
		if body["url"] == nil || body["is_vanity"] == nil || (body["url"] != nil && body["is_vanity"] != nil && reflect.TypeOf(body["url"]).String() != "string" || reflect.TypeOf(body["is_vanity"]).String() != "bool") {
			return c.JSON(fiber.Map{
				"status":      500,
...
func isValidUrl(toTest string) bool {
  ...
  if !urlRegex.MatchString(toTest) {
    return false
  }
